...
class Bomb:
    ...
    def fire(self, arr, perm, bomberman, en, level):
        ...
        for i in range(2*level+3):
            if(en[i] is not None and ((en[i].x == self.x and en[i].y == self.y) or (en[i].x == self.x+1 and en[i].y == self.y) or (en[i].x == self.x and en[i].y == self.y+1) or (en[i].x == self.x-1 and en[i].y == self.y) or (en[i].x == self.x and en[i].y == self.y-1))):
